...
class weapon_hlegon : ScriptBasePlayerWeaponEntity
{
	...
	void UpdateEffect( const Vector& in startPoint, const Vector& in endPoint, float timeBlend )
	{
		...
		Vector tempVec = ClassiscWeapon::getPlayerColor(m_pPlayer, COLOR_EGON);
		if ( m_fireMode == FIRE_WIDE )
			m_pBeam.SetColor( int(tempVec.x/2) + ( 25 * int( timeBlend ) ), int(tempVec.y/2) + ( 30 * int( timeBlend ) ), int(tempVec.z/2) + 80 * int( abs( sin( g_Engine.time * 10 ) ) ) );
		else
			m_pBeam.SetColor( int(tempVec.x/2) + ( 25 * int( timeBlend ) ), int(tempVec.y/2) + ( 30 * int( timeBlend ) ), int(tempVec.z/2) + 80 * int( abs( sin( g_Engine.time * 10 ) ) ) );
	...
	void CreateEffect()
	{
		...
		if ( m_fireMode == FIRE_WIDE )
		{
			...
			m_pBeam.SetNoise( 20 );
			m_pNoise.SetColor( int(tempVec.x), int(tempVec.y), int(tempVec.z) );
			m_pNoise.SetNoise( 8 );
		}
		else
		{
