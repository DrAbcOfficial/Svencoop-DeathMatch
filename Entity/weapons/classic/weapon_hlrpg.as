...
class CLaserSpot : ScriptBaseEntity
{
	...
	void Spawn()
	{
		...
		self.pev.rendercolor = ClassiscWeapon::getPlayerColor(m_pPlayer, COLOR_DOT);
...
class CRpgRocket : ScriptBaseEntity
{
	...
	void IgniteThink()
	{
		...
		// rocket trail
		uint8 r, g, b, a;
		Vector tempVec = ClassiscWeapon::getPlayerColor(cast<CBasePlayer@>(g_EntityFuncs.Instance(pev.owner)), COLOR_TRAIL);
		r = uint8(tempVec.x);
		g = uint8(tempVec.y);
		b = uint8(tempVec.z);
		a = 255;
