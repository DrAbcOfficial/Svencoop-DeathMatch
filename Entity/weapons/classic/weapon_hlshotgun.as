...
class weapon_hlshotgun : ScriptBasePlayerWeaponEntity
{
	...
	void ItemPostFrame()
	{
		if( m_flPumpTime != 0 && m_flPumpTime < g_Engine.time && m_fPlayPumpSound )
		{
			...
			for(int i = 0; i < (m_fIsDoubleFire == true ? 2 : 1); i++)
			{
				g_EntityFuncs.EjectBrass( m_pPlayer.GetGunPosition() + g_Engine.v_up * -6 + g_Engine.v_forward * 17 + g_Engine.v_right * 8, vecShellVelocity, m_pPlayer.pev.angles[1], m_iShell, TE_BOUNCE_SHOTSHELL );
			}
