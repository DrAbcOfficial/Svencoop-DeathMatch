namespace pvpHud
{
    ...
    CScheduledFunction@ SendHUDTimer = null;
    void MapInit()
    {
        if(SendHUDTimer is null)
            @SendHUDTimer = g_Scheduler.SetInterval( "SendHUDs", 1, g_Scheduler.REPEAT_INFINITE_TIMES );
        else
        {
            if(SendHUDTimer.HasBeenRemoved())
                @SendHUDTimer = g_Scheduler.SetInterval( "SendHUDs", 1, g_Scheduler.REPEAT_INFINITE_TIMES );
        }
    }
