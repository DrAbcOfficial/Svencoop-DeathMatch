namespace pvpConfig
{
    ...
    void SetConfigCallBack(const CCommand@ Argments)
    {
        ...
        switch(pvpUtility::getStringType(val))
        {
            ...
            case PDATA_ARRAY: tempAry = val.Replace("<ARRAY>","").Split("&|");setConfig(section, key, tempAry);break;
    ...
    pvpFile::CINIValue getConfig(string&in section, string&in key)
    {
        ...
        else
        {
            ...
            if(!tempConf.exists(key))
                ...
                return cast<CINIValue@>(tempConf[key]);
        }
        return CINIValue();
    }
    ...
    void setConfig(string&in section, string&in key, string&in val)
    {
        if(preConfig(section))
            return;
        dictionary(config[section]).set(key, CINIValue(val));
    }
    ...
    void setConfig(string&in section, string&in key, array<string>&in val)
    {
        if(preConfig(section))
            return;
        dictionary(config[section]).set(key, CINIValue(val));
